...
class Npwizard {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'verify_text'    => esc_html('verifying', 'restique'),
    ...
    public function menu_page() {
        add_theme_page(esc_html__('Theme Wizard', 'restique'), esc_html__('Theme Wizard', 'restique'), 'manage_options', $this->page_slug, array($this, 'wizard_page'));
    }
    ...
    public function wizard_page() {
        ...
        <div class="wrap npwizard-wrap-perent">
            ...
            if (defined('APP_PLUGIN_VERSION') && isset($GLOBALS['npThemeVersion']) && (float)APP_PLUGIN_VERSION > (float)$GLOBALS['npThemeVersion']) {
                ...
                echo sprintf('<div class="npwizard-warning"><p>%s</p></div>', 'The active theme has a version lower than the plugin version. Please update the theme too.', 'restique');
            }
            ?>
        </div><!-- .npwizard-wrap -->
    ...
    public function get_steps() {
        ...
        $steps = array(
            'plugins' => array(
                'id'			=> 'plugins',
                'title'			=> __('Install Theme Plugins', 'restique'),
                'icon'			=> 'admin-plugins',
                'view'			=> 'get_step_plugins',
                'callback'		=> 'install_plugins',
                'button_text'	=> __('Install Plugins', 'restique'),
                'can_skip'		=> false
            ),
            'content' => array(
                'id'			=> 'content',
                'title'			=> __('Import Content', 'restique'),
                'icon'			=> 'welcome-content-menus',
                'view'			=> 'get_step_content',
                'callback'		=> 'import_content',
                'callback2'		=> 'replace_content',
                'button_text'	=> __('Import Content', 'restique'),
                'button2_text'	=> __('Replace previously imported Content', 'restique'),
                'can_skip'		=> true,
                'can_replace'	=> true
            ),
            'done' => array(
                'id'			=> 'done',
                'title'			=> __('Your website is ready!', 'restique'),
                'icon'			=> 'yes',
                'view'			=> 'get_step_done',
                'callback'		=> ''
            )
    ...
    public function get_step_plugins() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Install plugins included with the Theme. <br>Click the "Install Plugins" button to start the installation.', 'restique')
        );
    ...
    public function get_step_content() {
        ...
        if($hideImport) {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Content has already been imported. Please skip this step', 'restique')
           );
        } else {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Theme has Pages, Images, Menu, Header, and Footer. </br></br>Do you want to import the Content?', 'restique')
            );
        }
        $content['import_options'] = sprintf(
            ...
            __('Import Sidebars Content', 'restique')
        );
        if (class_exists('Woocommerce')) {
            $content['import_options'] .= sprintf(
                ...
                __('Import Products Content to Woocommerce', 'restique')
            );
        }
    ...
    public function get_step_done() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Congratulations! The theme has been activated and your website is ready.', 'restique')
        );
        $content['summary'] .= sprintf( '<p>%s</p>', 'Create a new page with the Nicepage Editor.', 'restique' );
	...
	public function setup_plugins() {
		if (! check_ajax_referer('npwizard_nonce', 'wpnonce') || empty($_POST['slug'])) {
			wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'restique')));
		}
		$json = array();
		...
		foreach ($plugins['activate'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Activating Plugin', 'restique'),
		...
		foreach ($plugins['update'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Updating Plugin', 'restique'),
				);
				break;
			}
		}
		foreach ($plugins['install'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Installing Plugin', 'restique'),
				);
				break;
			}
		}
		if ($json) {
			...
			wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'restique')));
		}
		exit;
	}
    ...
    public function setup_content() {
        ...
        $json = array(
            ...
            'message'       => esc_html__('Import Content', 'restique'),
       );
        if ($json) {
            $json['hash'] = md5(serialize($json)); // used for checking if duplicates happen, move to next plugin
            wp_send_json($json);
