...
if (have_posts()) {
    ...
    if ($blog_sections_count) {
        for ($template_idx = 0; $template_idx < $templates_count; $template_idx++) {
            ...
            $translations = '';
            if ($language) {
                if (file_exists(get_template_directory() . '/template-parts/'. $blog_custom_template . '/translations/' . $language .'/post-content-' . ($template_idx + 1) . '.php')) {
                    $translations = '/translations/' . $language;
                }
            }
            $blog_content_path = get_template_directory() . '/template-parts/'. $blog_custom_template . $translations . '/post-content-' . ($template_idx + 1) . '.php';
            ob_start();
